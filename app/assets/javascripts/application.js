//= require jquery
//= require jquery_ujs
//= require bootstrap-sprockets

//= require rs-plugin/js/jquery.themepunch.enablelog
//= require rs-plugin/js/jquery.themepunch.revolution
//= require rs-plugin/js/jquery.themepunch.revolution.min
//= require rs-plugin/js/jquery.themepunch.tools.min


//= require js/pace.min
//= require js/isotope-custom
//= require js/jqBootstrapValidation
//= require js/jquery-migrate.min
//= require js/jquery.imagesloaded.min
//= require js/jquery.isotope.min
//= require js/jquery.magnific-popup.min
//= require js/jquery.masonry.min
//= require js/jquery.stellar.min
//= require js/masonary-custom
//= require js/masonry.pkgd.min
//= require js/medical-custom
//= require js/modernizr.custom.97074
//= require js/revolution-custom
//= require js/waypoints.min
//= require js/wow.min

//= require_tree .
